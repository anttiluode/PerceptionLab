...
class NodeConfigDialog(QtWidgets.QDialog):
    def __init__(self, node_item, parent=None):
        ...
        config_options = self.node.get_config_options()
        if not isinstance(config_options, list):
             print(f"Warning: get_config_options for {self.node.node_title} returned invalid data. Should be a list.")
             config_options = []

        for item in config_options:
            if not isinstance(item, (list, tuple)) or len(item) != 4:
                print(f"Warning: Invalid config option in {self.node.node_title}. Skipping.")
                continue
                
            display_name, key, current_value, options = item
            
            h_layout = QtWidgets.QHBoxLayout()
            h_layout.addWidget(QtWidgets.QLabel(display_name + ":"))

            if options == "file_open":
                # --- NEW FILE DIALOG LOGIC ---
                self.inputs[key] = QtWidgets.QLineEdit(str(current_value))
                browse_btn = QtWidgets.QPushButton("Browse...")
                
                # We need to pass the QLineEdit to the dialog function
                browse_btn.clicked.connect(lambda _, k=key, w=self.inputs[key]: self.open_file_dialog(k, w))
                
                h_layout.addWidget(self.inputs[key], 1)
                h_layout.addWidget(browse_btn)
                # --- END NEW LOGIC ---

            elif isinstance(options, (list, tuple)):
                # --- DROPDOWN (COMBO) LOGIC ---
                combo = QtWidgets.QComboBox()
                for option_item in options:
                    if isinstance(option_item, (list, tuple)) and len(option_item) == 2:
                        name, value = option_item
                    else:
                        # Handle simple lists (like ['A', 'B'])
                        name = str(option_item)
                        value = option_item
                ...
                if isinstance(current_value, (int, float)) and not any(v == current_value for _, v in options):
                     if combo.findData(current_value) == -1: # Avoid duplicates
    ...
    def open_file_dialog(self, key, line_edit_widget):
        # --- NEW HELPER METHOD ---
        # Opens a file dialog and puts the path into the line edit
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Select File", "", "All Files (*)")
        
        if fileName:
            line_edit_widget.setText(fileName)
            # We also update the node's config *immediately*
            setattr(self.node, key, fileName)
            if hasattr(self.node, '_load_image'):
                 self.node._load_image() # Tell LoadImageNode to reload
