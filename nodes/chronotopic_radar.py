"""
Standalone Chronotopic Radar Node v9: Dual-Mode Interferometry
=====================================================================
Antti Luode (PerceptionLab) | 2026

Self-standing Perception Lab node for the Deerskin Time-Folded Surface.
Now includes Aperture Toggling to allow pure Eigenmode projection into
FFT Cochlea nodes without the circular diffraction (sinc) artifacts.

Features:
- Native EDF file loading and MNE preprocessing
- Lobe Selection (All, Occipital, Frontal, Temporal, Parietal, Central)
- Aperture Shape (Circle/Lens vs Square/Pure Field)
...
class StandaloneChronotopicRadarNode(BaseNode):
    ...
    def __init__(self):
        ...
        self.use_circular_mask = 1  # 1 = Circle, 0 = Square (Pure Field)
        self.amp_slider = 0.0       # -10 to 70 
        self.blur = 0.85            # Temporal decay 
    ...
    def _init_geometry(self):
        ...
        # Calculate pure inverse-distance weights for the entire square
        raw_weights = np.zeros((len(self.grid_coords), self.n_ch))
        for i, (name, coord) in enumerate(ELEC_COORDS_2D.items()):
            dists = np.linalg.norm(self.grid_coords - np.array(coord), axis=1)
            raw_weights[:, i] = 1.0 / (dists**3 + 0.05)
            
        raw_weights /= raw_weights.sum(axis=1, keepdims=True)
        
        # Store both the unmasked (Square) and masked (Circle) weights
        self.elec_weights_unmasked = raw_weights.copy()
        
        self.elec_weights_masked = raw_weights.copy()
        self.elec_weights_masked[~self.mask] = 0.0

    def get_config_options(self):
        region_options =[(r, r) for r in EEG_REGIONS.keys()]
        mask_options =[("Circle (Diffraction Lens)", 1), ("Square (Pure Eigenmodes)", 0)]
        
        return[
            ("EDF File Path", "edf_path", self.edf_path, 'string'),
            ("Brain Region (Lobe)", "selected_region", self.selected_region, region_options),
            ("Aperture Shape", "use_circular_mask", self.use_circular_mask, mask_options),
    ...
    def set_config_options(self, options):
        if isinstance(options, dict):
            ...
            if 'selected_region' in options and options['selected_region'] != self.selected_region:
                self.accum_color.fill(0)
            if 'use_circular_mask' in options and options['use_circular_mask'] != self.use_circular_mask:
                self.accum_color.fill(0)
    ...
    def step(self):
        ...
        if current_path != self._last_path or self.selected_region != self._last_region:
        ...
        # CHOOSE WEIGHTS BASED ON APERTURE SETTING
        active_weights = self.elec_weights_masked if self.use_circular_mask else self.elec_weights_unmasked
        ...
        if self.mode == 1:
            ...
            target_color[:, 2] = np.dot(active_weights, norm_fast) 
            target_color[:, 1] = np.dot(active_weights, norm_med)   
            target_color[:, 0] = np.dot(active_weights, norm_slow)  
            ...
            wire_pixel = np.dot(active_weights, wire_norm)
            outer_pixel = np.dot(active_weights, outer_norm)
        ...
        # APPLY APERTURE MASK IF IN CIRCLE MODE
        ...
        if self.use_circular_mask:
            ...
            # Draw a subtle square boundary for the Pure Field mode
            cv2.rectangle(img_bgr, (2, 2), (self.res-3, self.res-3), (82, 61, 52), 2)
