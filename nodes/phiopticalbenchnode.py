"""
Phi Fractal Bench (Final Production Fix)
=======================================
"The Eye of the Beholder"

FIXED:
- Implemented Lazy Loading in step() to sync with JSON restoration.
- Removed direct load_data() from set_config_options to prevent Host desync.
- Added self._last_path to ensure the node only loads the EDF file once.
"""
...
class PhiOpticalBenchNode(BaseNode):
    ...
    def rebuild_grid(self):
        """Re-initializes the pixel coordinate system."""
    ...
    def load_file_dialog(self):
        ...
        if fname:
            self.file_path = fname # Setting this triggers lazy load in step()
    ...
    def step(self):
        ...
        field = np.sum(fft_res[:, mask], axis=1) 
        ...
        for start_i in range(0, self.n_ch, batch_size):
            ...
            batch_emitters = self.emitters[start_i:end_i] 
            batch_field = field[start_i:end_i]            
            ex, ey, ez = batch_emitters[:, 0][:, None, None], batch_emitters[:, 1][:, None, None], batch_emitters[:, 2][:, None, None]
            dx, dy, dz = self.pixel_x - ex, self.pixel_y - ey, self.focus_z - ez
            dists = np.sqrt(dx**2 + dy**2 + dz**2)
            waves = batch_field[:, None, None] * np.exp(-1j * self.lens_k * dists)
            waves *= (1.0 / (dists + 0.1))
    ...
    def set_config_options(self, options):
        """CRITICAL: Only set the path. Do NOT call load_data here."""
